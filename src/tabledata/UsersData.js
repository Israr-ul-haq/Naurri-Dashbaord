...
export const columns = (data, service, setLoader) => {
  return [
    ...
    },
    {
      name: "Status",
      cell: (row) =>
        row["userStatusId"] === StatusCodes.active ? (
          <Box color={Colors.header_success_color}>
            {row["userStatusTitle"]}
          </Box>
        ) : row["userStatusId"] === StatusCodes.block ? (
          <Box color={Colors.header_danger_color}>{row["userStatusTitle"]}</Box>
        ) : (
          <Box color={Colors.header_warning_color}>
            {row["userStatusTitle"]}
          </Box>
        ),
      sortable: true,
    },
    {
      ...
      cell: (row) => (
        <Flex alignItems={"center"}>
          <Link style={{ marginRight: "10px" }} to={`/users/${row["id"]}`}>
            <Img
          ...
          <Link style={{ marginRight: "10px" }} to={`/users/edit/${row["id"]}`}>
            <Img
              placeholder={loadingImage}
